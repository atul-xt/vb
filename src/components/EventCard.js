import { Heart, MapPin, Calendar, DollarSign, Users } from "lucide-react"

export default function EventCard({ data }) {
    const { id, title, desc, imageUrl, date, highlight, address, by } = data;
    return (
        <div className="flex items-center justify-center p-4 gap-8">
            {data?.map((puja) => (
                <div key={puja.id} className="w-full max-w-xs bg-white rounded-2xl shadow-lg overflow-hidden relative transform -rotate-5">
